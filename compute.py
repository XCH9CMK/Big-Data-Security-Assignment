...
def compute_WER(audio_file=None, reference_text=None):
    """
    计算错词率（WER）
    
    Args:
        audio_file: 音频文件路径，如果为None则自动查找项目生成的音频文件
        reference_text: 参考文本，如果为None则使用默认文本
    ...
    if audio_file is None:
        ...
        if audio_file is None:
            raise FileNotFoundError("未找到音频文件，请先运行实验生成音频数据或指定audio_file参数")
